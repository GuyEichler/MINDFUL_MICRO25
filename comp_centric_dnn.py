...
if __name__ == "__main__":
  ...
  log_file = open("logs/log_dnn_output.txt", "w")
  ...
  with open('socs_intersections.py', 'w') as file:
    file.write(f'#Intersections of power budget\n')
  ...
  for soc in socs:
    ...
    for i in range(len(dnn_types)):
      ...
      x_inter, y_inter, ni_channels, sensing_power_consumption, non_sensing_power_consumption, total_power_consumption, total_power_budget = \
        scaling_dnn_comp(soc[0], dnn_arch, physical_specs45, soc[1], show=False, maximum_channels=max_ch, step=step)

      name = soc[0]["Name"]

      np.savetxt(f'data/{plot_name}_comp_data{name}.txt', np.column_stack((total_power_budget, total_power_consumption, ni_channels)))
      ...
      if i == len(dnn_types)-1:
        ...
        for intersection in soc[0]["budget_cutoff"]:
          if len(intersection) != 0:
            ...
            intersections_list.append(None)
        with open('socs_intersections.py', 'a') as file:
          file.write(f"list_{name} = [")
