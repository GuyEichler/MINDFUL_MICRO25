...
def scaling_dnn_comp(soc, dnn_arch, physical_specs, comm_specs, show=True, maximum_channels=2368, step=4):
  ...
  for channels in channel_numbers:
    ...
    if channels <= orig_channels:
      ...
      if channels == 0:
        ...
        non_sensing_power_consumption_plot.append(None)
      ...
      physical["mac_area"] = 0.000783 * 1.5 #45nm
      ...
      # non_sensing_power_no_comm = (power_per_channel_comm * orig_channels - communication_power_orig * 10**3) / orig_channels * arch["last_output"]
      ...
      # communication_power_orig, _ = calc_communication(1024, 0, physical, comm, enable_qam=False)
      # non_sensing_power_no_comm_orig = calc_non_sensing_power_no_comm(physical, comm, soc, orig_channels)

      non_sensing_power_comm = None #non_sensing_power_no_comm + communication_power * 10**3

      # non_sensing_power_comm_orig = non_sensing_power_no_comm_orig + communication_power_orig * 10**3

      # res = res - non_sensing_power_comm
      # print("LAST OUTPUT: ", arch["last_output"])
      # print("COMMUNICATION POWER:", communication_power*10**3)
      # print("NON SENSING POWER NO COMM:", non_sensing_power_no_comm)
      # print("NON SENSING POWER NO COMM ORIG:", non_sensing_power_no_comm_orig)

      #Update to include also the reset of the non sensing power
      # res = res + non_sensing_power_no_comm

      non_sensing_power_consumption_plot.append(res)

      computation_area = physical["mac_area"] * macs

      #power consumption
      non_sensing_power_consumption = res
  ...
  if show == True:
    ...
    label_x, label_y = x_axis[mid_index-120], total_power_budget_plot[mid_index-80]

    if dnn_type == "DN-CNN":
      angle = np.arctan2(total_power_budget_plot[mid_index+1] - total_power_budget_plot[mid_index-1], x_axis[mid_index+1] - x_axis[mid_index-1]) * 20 #/ np.pi
    else:
      angle = np.arctan2(total_power_budget_plot[mid_index+1] - total_power_budget_plot[mid_index-1], x_axis[mid_index+1] - x_axis[mid_index-1]) * 25 #/ np.pi
    ...
    if dnn_type == "DN-CNN":
      ax.axvline(x=1388, ymin=0, ymax=0.4, color='gray', linestyle='--', linewidth=2, label=None)
    else:
      ax.axvline(x=1758, ymin=0, ymax=0.53, color='gray', linestyle='--', linewidth=2, label=None)
    ...
    if dnn_type == "DN-CNN":
      plt.plot(x_axis, non_sensing_power_consumption_plot, label=f"Non-Sensing Power ({dnn_type})", color='purple', linewidth=3)
    else:
      plt.plot(x_axis, non_sensing_power_consumption_plot, label=f"Non-Sensing Power ({dnn_type})", color='purple', linewidth=3)
    ...
    plt.savefig(f"figures/{name}_{dnn_type}_comp_scale.pdf")
